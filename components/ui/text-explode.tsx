...
const characterVariants: Variants = {
  ...
  end: { translateY: 0, translateX: 0, rotate: 0, scale: 1 },
  initial: { opacity: 1 },
};
...
export default function TextExplode({
  ...
  const animateSequence = useCallback(async () => {
    await Promise.all([
      controls.start("shrink", { duration: 1.4, ease: "easeOut" }),
      controls.start("jitter", { delay: 0.3 }),
    ]);
    await controls.start("explode", { duration: 0.6 });
    await controls.start("end");
    await controls.start("initial", { delay: 1.2, duration: 1.2, type: "spring" });
  ...
  useEffect(() => {
    if (!characters.length || mode === "hover") return;
    animateSequence();
